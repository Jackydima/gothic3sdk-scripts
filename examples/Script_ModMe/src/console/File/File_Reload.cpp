...
bCString CMD_File_Sub::Reload(bTObjArray<bCString> a_arrParams)
{
    if (s_Reloadables.IsValidKey(a_arrParams[0]))
    {
        ...
        return bCString::GetFormattedString("Reloaded: %s", a_arrParams[0].GetText());
    }

    eCResourceAdmin *pResourceAdmin =
        dynamic_cast<eCResourceAdmin *>(eCModuleAdmin::GetInstance().FindModule("eCResourceAdmin"));
    eCResourceDataEntity *pResourceDataEntity =
        pResourceAdmin->QueryResourceDataEntity(a_arrParams[0], static_cast<eEResourceCacheBehavior>(3));
