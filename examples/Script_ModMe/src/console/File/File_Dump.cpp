...
bCString CMD_File_Sub::Dump(bTObjArray<bCString> a_arrParams)
{
    ...
    if (!File.IsValid())
        return bCString::GetFormattedString("$File '%s' does not exist.", FilePath.GetText());
    if (!File.Open(eCArchiveFile::bEArchiveFileAccessMode_Read, GEFalse))
        return bCString::GetFormattedString("$Could not open the file '%s'.", FilePath.GetText());
    ...
    return bCString::GetFormattedString("Dumped '%s' to '%s'", FilePath.GetText(), DumpFile.GetFilePath().GetText());
}
