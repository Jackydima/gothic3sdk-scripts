...
bCString CreateCollisionMesh(bTObjArray<bCString> const &a_arrParams, GEBool a_bConvex)
{
    ...
    if (ResourceCompiler::CreateCollisionMesh(strMeshName, strColMeshPath, a_bConvex, &Scaling))
        return bCString::GetFormattedString(
            "Created %s CollisionMesh '%s' for the mesh '%s' with scaling %.4f/%.4f/%.4f.", strMeshType,
            strColMeshPath.GetText(), strMeshName.GetText(), Scaling.GetX(), Scaling.GetY(), Scaling.GetZ());

    return bCString::GetFormattedString("$Creation of %s CollisionMesh '%s' for the mesh '%s' failed.", strMeshType,
                                        strColMeshPath.GetText(), strMeshName.GetText());
}
} // namespace
