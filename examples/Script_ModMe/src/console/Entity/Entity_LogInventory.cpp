...
bCString CMD_Entity_Sub::LogInventory(bTObjArray<bCString> a_arrParams)
{
    ...
    for (GEInt i = 0; i < Inventory->GetStackCount(); i++)
    {
        ...
        bCString strType = pStack->GetType().GetValue() == gEStackType_Normal
                             ? "Normal"
                             : (pStack->GetType().GetValue() == gEStackType_Treasure ? "Treasure" : "Merchandise");

        Logger.LogFormat("%d - %s x %d%s: Type = %s, Quality = %d, UseType = %d, Slot = %d, QuickSlot = %d, Category = "
                         "%d, CombinedQuality = %d, Learned = %d, SortIndex = %d, ActivationCount = %d, "
                         "TransactionAmount = %d, TransactionCount = %d, TransferAmount = %d, TransferRemainder = %d\n",
                         i, strName.GetText(), pStack->GetAmount(), pItem ? "" : " (No Item)", strType.GetText(),
                         pStack->GetQuality(), pStack->GetUseType(), pStack->GetSlot().GetValue(),
                         pStack->GetQuickSlot(), pStack->GetCategory(), pStack->GetCombinedQuality(),
                         !!pStack->GetLearned(), pStack->GetSortIndex(), pStack->GetActivationCount(),
                         pStack->GetTransactionAmount(), pStack->GetTransactionCount(), pStack->GetTransferAmount(),
                         pStack->GetTransferRemainder());
    }
