...
bCString CMD_Entity_Sub::Enable(bTObjArray<bCString> a_arrParams)
{
    ...
    if (!pEntity)
        return bCString::GetFormattedString("$Unable to find entity with the name %s.", a_arrParams[0].GetText());

    GEBool bEnable = a_arrParams[1].GetBool();

    if (pEntity->IsEnabled() == bEnable)
        return bCString::GetFormattedString("$Status of entity %s was already %s.", pEntity->GetName().GetText(),
                                            bEnable ? "on" : "off");

    pEntity->Enable(bEnable);
    return bCString::GetFormattedString("Changed status of entity %s to %s.", pEntity->GetName().GetText(),
                                        bEnable ? "on" : "off");
}
