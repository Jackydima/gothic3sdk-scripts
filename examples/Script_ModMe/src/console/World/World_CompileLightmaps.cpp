...
GEBool GE_STDCALL ComputeStaticLightData(eCEntity *a_pEntity, GEBool a_bUnk)
{
    ...
    if (Result && a_pEntity)
    {
        ...
        bUsingLightmap = pMeshBase && pMeshBase->GetResourceDataEntity()
                      && pMeshBase->GetStaticLightingType() == eEStaticLighingType_Lightmap;
        ...
        if (pIlluminated && pIlluminated->GetStaticIlluminated() == eEStaticIlluminated_Static && !bUsingLightmap)
        {
            ...
            ToFile(StreamOut,
                   "Data\\StaticLightdata\\" + a_pEntity->GetName() + "_" + a_pEntity->GetID().GetText() + ".sldat");
            mCLoggingAdmin<mCFileLogger>::GetLogger("logs\\StaticLightdata.log")
                .LogFormat("Writing %s_%s_.sldat\n", a_pEntity->GetName().GetText(),
                           a_pEntity->GetID().GetText().GetText());
        }
    }
    return Result;
}
...
GEBool GE_STDCALL ComputeStaticLighting(eCEntity *a_pEntity, GEBool a_bUnk1, GEBool a_bUnk2, GEBool a_bUnk3,
                                        GEBool a_bUnk4)
{
    if (bEnableEntityCheck && !EntityCompileBox.Intersects(a_pEntity->GetWorldNodeBoundary()))
        return GETrue;

    return HookComputeStaticLighting.GetOriginalFunction(&ComputeStaticLighting)(a_pEntity, a_bUnk1, a_bUnk2, a_bUnk3,
                                                                                 a_bUnk4);
}
...
void InitHooks()
{
    ...
    if (!HookComputeStaticLightData.IsActive())
        HookComputeStaticLightData
            .Prepare(PROC_Engine("?ComputeStaticLightData@eCStaticLightPreprocessor@@QAE_NPAVeCEntity@@_N@Z"),
                     &ComputeStaticLightData, mCBaseHook::mEHookType_ThisCall)
            .Hook();

    if (!HookComputeStaticLighting.IsActive())
        HookComputeStaticLighting.Hook(
            PROC_Engine("?ComputeStaticLighting@eCStaticLightPreprocessor@@QAE_NPAVeCEntity@@_N111@Z"),
            &ComputeStaticLighting, mCBaseHook::mEHookType_ThisCall);

    static mCCallHook HookUnmountDir;
    if (!HookUnmountDir.IsActive())
        HookUnmountDir.Hook(RVA_Importer(0x2B02F), &UmountDir, mCBaseHook::mEHookType_ThisCall);

    static mCCallHook HookMountDir;
    if (!HookMountDir.IsActive())
...
bCString CMD_World_Sub::CompileLightmaps(bTObjArray<bCString> a_arrParams)
{
    ...
    if (a_arrParams.GetCount() >= 2 && a_arrParams.GetCount() <= 4)
    {
        ...
        if (a_arrParams.GetCount() < 4)
        {
            ...
            if (Origin == None)
                return bCString::GetFormattedString("$Could not find a entity with the name %s.",
            ...
            AreaMessage += bCString::GetFormattedString(
                "Only Entities within [min: %f/%f/%f//, max: %f/%f/%f//] will be taken into account.\n",
                EntityCompileBox.GetMin().GetX(), EntityCompileBox.GetMin().GetY(), EntityCompileBox.GetMin().GetZ(),
                EntityCompileBox.GetMax().GetX(), EntityCompileBox.GetMax().GetY(), EntityCompileBox.GetMax().GetZ());
        }
        else
        {
        ...
        AreaMessage += bCString::GetFormattedString("Compiling Lightmaps only for the area (x: %d to %d, z: %d to %d) "
                                                    "which is contained in the cells (x: %d to %d, z: %d to %d).\n",
                                                    iMinPositionX, iMaxPositionX, iMinPositionZ, iMaxPositionZ,
                                                    iMinGridIndexX, iMaxGridIndexX, iMinGridIndexZ, iMaxGridIndexZ);
        GetDefaultLogger().LogString(AreaMessage);
    }

    gCWorld *pWorld = gCWorld::GetCurrentWorld();
    iCSceneCompiler SceneCompiler;
    bEResult Result = SceneCompiler.CompileWorld(pWorld->GetFileBaseName(),
                                                 iCSceneCompiler::iECompileOptions_CompileLightmaps, GEFalse);
