...
bCString CMD_World_Sub::SaveSector(bTObjArray<bCString> a_arrParams)
{
    ...
    return bCString::GetFormattedString(
        "%s and %i dynamic layer(s) have been successfully saved. Skipped %i non-physical file(s).",
        pSector->GetFileBaseName().GetText(), (pSector->GetDynamicLayerCount() - iSkippedFileCount), iSkippedFileCount);
}
