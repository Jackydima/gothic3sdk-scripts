...
bCString PinEntity(bTObjArray<bCString> a_arrParams)
{
    ...
    eCEntity *pEntity = dynamic_cast<eCEntity *>(
        s_SceneAdmin.GetEntity(bCPropertyID(bCGuid(a_arrParams[0])), eEGetEntityTypeHint_Entity));
    if (!pEntity)
...
void LogProcessingRangeRegions(eCSceneAdmin &a_SceneAdmin)
{
    ...
    Logger.LogFormat("ProcessingRangeRegion: %s\n",
                     BoxToString(*reinterpret_cast<bCBox *>(pEntityAdmin + 0x64)).GetText());
    Logger.LogFormat("OldProcessingRangeRegion: %s\n",
                     BoxToString(*reinterpret_cast<bCBox *>(pEntityAdmin + 0x7C)).GetText());
    Logger.LogFormat("StaticProcessingRangeRegion: %s\n",
                     BoxToString(*reinterpret_cast<bCBox *>(pEntityAdmin + 0x94)).GetText());
}

void LogStaticEntity(eCSceneAdmin &a_SceneAdmin, eCEntity const &a_Entity)
{
    mCFileLogger &Logger = mCLoggingAdmin<mCFileLogger>::GetLogger(g_strLogFile);
    Logger.LogFormat("%s | %s | %s | %s\n", a_Entity.GetName().GetText(), a_Entity.GetID().GetText().GetText(),
                     VectorToString(a_Entity.GetWorldPosition(), GEFalse).GetText(),
                     BoxToString(a_Entity.GetWorldNodeBoundary()).GetText());

    GEBool bIntersectsProcessingRangeRegion =
        a_SceneAdmin.GetAccessToEntityAdmin().IntersectsProcessingRangeRegion(a_Entity.GetWorldNodeBoundary());
    Logger.LogFormat("IntersectsProcessingRange: %d | WorldNodeBoundaryValid: %d | CachedIn: %d\n",
                     bIntersectsProcessingRangeRegion, a_Entity.GetEntityFlags().m_WorldNodeBoundaryValid,
...
void LogDynamicEntity(eCSceneAdmin &a_SceneAdmin, eCEntity const &a_Entity)
{
    ...
    Logger.LogFormat("Enabled: %d | Killed: %d | DeactivationEnabled: %d\n", a_Entity.GetEntityFlags().m_Enabled,
                     a_Entity.GetEntityFlags().m_Killed, a_Entity.GetEntityFlags().m_DeactivationEnabled);
}

GEBool InProcessingRange(eCSceneAdmin &a_SceneAdmin, eCEntity const &a_Entity)
{
    GEBool bPinnedNavigation =
        g_bPinNavigationEntities
        && (a_Entity.HasPropertySet(eEPropertySetType_NavZone) || a_Entity.HasPropertySet(eEPropertySetType_NavPath)
            || a_Entity.HasPropertySet(eEPropertySetType_NegZone)
            || a_Entity.HasPropertySet(eEPropertySetType_PrefPath));

    return bPinnedNavigation
        || a_SceneAdmin.GetAccessToEntityAdmin().IntersectsProcessingRangeRegion(a_Entity.GetWorldNodeBoundary());
}

GEBool CheckStaticEntity(eCSceneAdmin &a_SceneAdmin, eCEntity const &a_Entity)
{
    return InProcessingRange(a_SceneAdmin, a_Entity) && a_Entity.GetEntityFlags().m_WorldNodeBoundaryValid
        && a_Entity.GetEntityFlags().m_CachedIn;
...
{
    return a_Entity.GetEntityFlags().m_WorldNodeBoundaryValid && a_Entity.GetEntityFlags().m_CachedIn
        && a_Entity.GetEntityFlags().m_Enabled && !a_Entity.GetEntityFlags().m_Killed
        && (!a_Entity.GetEntityFlags().m_DeactivationEnabled || InProcessingRange(a_SceneAdmin, a_Entity));
    //&& pEntity->GetEntityFlags().m_ProcessingRangeEntered;
}
...
ME_MODULE_DEPENDS(DummyCollector, {"CommandHandler"})
{
    ...
    Hook_CollectProcessingRange.Hook(PROC_Game("?CollectProcessingRange@gCDummyCollectorAdmin@@IAEXXZ"),
                                     &CollectProcessingRange, mCBaseHook::mEHookType_ThisCall);
    Hook_GetEntitiesInProcessingRange.Hook(RVA_Game(0x3F664), &GetEntitiesInProcessingRange,
    ...
    // 110.0f
    static mCCallHook Hook_OCF_CollisionCircle_1(RVA_Game(0x43DA8), &SetObjectCullFactor,
    mCBaseHook::mEHookType_ThisCall); static mCCallHook Hook_OCF_CollisionCircle_2(RVA_Game(0x4424F),
    &SetObjectCullFactor, mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_OCF_CollisionShape(RVA_Game(0x45B0A), &SetObjectCullFactor, mCBaseHook::mEHookType_ThisCall);

    // 110.0f
    static mCCallHook Hook_OCF_DynamicCollisionCircle(RVA_Game(0x462BE), &SetObjectCullFactor,
    mCBaseHook::mEHookType_ThisCall);
    ...
    // Default: GETrue
    static mCCallHook Hook_PROFE_NavZone_1(RVA_Game(0x498EB), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);
    static mCCallHook Hook_PROFE_NavZone_2(RVA_Game(0x49DF4), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);
    static mCCallHook Hook_PROFE_NavZone_3(RVA_Game(0x4A12C), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_NavPath_1(RVA_Game(0x4816E), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);
    static mCCallHook Hook_PROFE_NavPath_2(RVA_Game(0x48523), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);
    static mCCallHook Hook_PROFE_NavPath_3(RVA_Game(0x488C9), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_NegZone_1(RVA_Game(0x4AEAE), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);
    static mCCallHook Hook_PROFE_NegZone_2(RVA_Game(0x4B57E), &SetProcessingRangeOutFadingEnabled,
                                           mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_PrefPath(RVA_Game(0x4C8AD), &SetProcessingRangeOutFadingEnabled,
                                          mCBaseHook::mEHookType_ThisCall);

    /*static mCCallHook Hook_PROFE_Anchor(RVA_Game(0x43463), &SetProcessingRangeOutFadingEnabled,
    mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_CollisionCircle_1(RVA_Game(0x43DBB), &SetProcessingRangeOutFadingEnabled,
    mCBaseHook::mEHookType_ThisCall); static mCCallHook Hook_PROFE_CollisionCircle_2(RVA_Game(0x44262),
    &SetProcessingRangeOutFadingEnabled, mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_CollisionShape(RVA_Game(0x45B1D), &SetProcessingRangeOutFadingEnabled,
    mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_DynamicCollisionCircle(RVA_Game(0x462D1), &SetProcessingRangeOutFadingEnabled,
    mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_Freepoint(RVA_Game(0x46894), &SetProcessingRangeOutFadingEnabled,
    mCBaseHook::mEHookType_ThisCall);

    static mCCallHook Hook_PROFE_NavOffset_1(RVA_Game(0x474BF), &SetProcessingRangeOutFadingEnabled,
    mCBaseHook::mEHookType_ThisCall); static mCCallHook Hook_PROFE_NavOffset_2(RVA_Game(0x4783E),
    &SetProcessingRangeOutFadingEnabled, mCBaseHook::mEHookType_ThisCall);*/
}
