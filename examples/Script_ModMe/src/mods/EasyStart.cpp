...
GEBool OpenTutorial(bCUnicodeString const &a_Message)
{
    ...
    {
        if (gCPlayerMemory_PS *pPlayerMemory = GetPropertySet<gCPlayerMemory_PS>(pHero, eEPropertySetType_PlayerMemory))
...
ME_MODULE(EasyStart)
{
    ...
    HookOpenTutorial.Hook(PROC_Game("?OpenTutorial@gCGUIManager@@QAE_NABVbCUnicodeString@@@Z"), &OpenTutorial,
    ...
    if (bAutostart)
    {
        /*
            Wird über
           eCApplication::OnIdle().eCApplication::OnRun().eCApplication::Process().eCModuleAdmin::Process().gCScriptAdmin::Process().gCScriptAdmin::LoadAllDLLs()
            aufgerufen. Wir klemmen uns hinter den eCApplication::Process() Aufruf in eCApplication::OnRun().
        */
        Hook_AfterApplicationProcess.Prepare(RVA_Engine(0x1677C), &AfterApplicationProcess).InsertCall().Hook();
    }
}
