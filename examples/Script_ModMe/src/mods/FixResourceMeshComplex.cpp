...
{
    a_Logger.LogFormat("Fixing %s...\n", a_strMeshName.GetText());

    eCResourceDataEntity *pMeshData =
        FindModule<eCResourceAdmin>()->QueryResourceDataEntity(a_strMeshName, eEResourceCacheBehavior_Immediate);
    if (!pMeshData)
    ...
    for (GEInt i = 0; i < pMesh->GetMeshResourceCount(); i++)
    {
        ...
        a_Logger.LogFormat("MeshElement %d (%s)\n", i, pMeshElement->GetMaterialName().GetText());
        a_Logger.PushIndent();
