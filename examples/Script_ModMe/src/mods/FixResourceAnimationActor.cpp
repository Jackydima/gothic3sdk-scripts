...
void eCWrapper_emfx2Actor_ReadAmbientOcclusion(/*bTObjArray<bTValArray<GEU32>> *o_arrData@<edi>, */ bCIStream *a_pInput)
{
    ...
    Hook_eCWrapper_emfx2Actor_ReadAmbientOcclusion.GetOriginalFunction (&eCWrapper_emfx2Actor_ReadAmbientOcclusion)(
        a_pInput);
...
bCString CalculateAmbientOcclusion(bCString const &a_strActorName, bCString const &a_strTargetActorName)
{
    ...
    eCResourceDataEntity *pActorData =
        FindModule<eCResourceAdmin>()->QueryResourceDataEntity(a_strActorName, eEResourceCacheBehavior_Immediate);
    if (!pActorData)
    ...
    GE_ARRAY_FOR_EACH (Fixup, AmbientOcclusionFixups)
    {
        ...
        if (SrcFile.GetPosition() != DstFile.GetPosition())
            return bCString::GetFormattedString(
                "Failed because the size of the ambient occlusion data at offset %x has changed.", Fixup.uOffset);
    }
...
ME_MODULE(FixResourceAnimationActor)
{
    ...
    Hook_eCWrapper_emfx2Actor_Read.Hook(PROC_Engine("?Read@eCWrapper_emfx2Actor@@UAE?AW4bEResult@@AAVbCIStream@@@Z"),
                                        &eCWrapper_emfx2Actor_Read, mCBaseHook::mEHookType_ThisCall);
    Hook_eCWrapper_emfx2Actor_ReadAmbientOcclusion.Hook(RVA_Engine(0x4FFB0), &eCWrapper_emfx2Actor_ReadAmbientOcclusion,
                                                        mCBaseHook::mEHookType_Mixed, mERegisterType_Edi);
    ...
    GE_ARRAY_FOR_EACH (FileName, arrFiles)
    {
        ...
        Logger.LogFormat("Calculating ambient occlusion for %s...", FileName.GetText());
