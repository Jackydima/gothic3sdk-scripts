...
GEBool CreateCollisionMesh(bCString const &a_strMeshName, bCString const &a_strColMeshPath, GEBool a_bConvex,
                           bCVector const *a_Scaling)
{
    eCResourceAdmin *pResourceAdmin =
        dynamic_cast<eCResourceAdmin *>(eCModuleAdmin::GetInstance().FindModule("eCResourceAdmin"));
    eCResourceDataEntity *pMeshData =
        pResourceAdmin->QueryResourceDataEntity(a_strMeshName, eEResourceCacheBehavior_Immediate);
    ...
    if (pMeshData && pColShapeData)
    {
        ...
        if (a_Scaling)
        {
            ...
            {
                // Erstmal Collision Mesh mit einer Skalierung von 1.0f erstellen und danach Kopie mit der richtigen
                // Skalierung erzeugen.
        ...
        eCResourceCollisionMesh_PS *pTempColShape =
            dynamic_cast<eCResourceCollisionMesh_PS *>(pColShapeData->SearchResourcePropertySet());
        if (pTempColShape)
        {
            ...
            eCResourceCollisionMesh_PS *pScaledColShape =
                dynamic_cast<eCResourceCollisionMesh_PS *>(pScaledColShapeData->SearchResourcePropertySet());
            if (!pScaledColShape)
                return GEFalse;

            // ConvexHull hat immer nur ein SubMesh (zumindest in den Originaldaten)
            // Wenn CopyWithScaling für ein Mesh mit n SubMeshes aufgerufen wird, haben die IndexedBoundarys
            // fälschlicherweise ein Scaling von scaling^n, da die Berechnung in jedem Schleifendurchlauf ausgeführt
            // wird. Gilt auch für alle anderen Berechnungen?!
            if (bUniformScaling)
                pScaledColShape->CopyWithScaling(*pTempColShape, fUniformScaling);
            else
                pScaledColShape->CopyWithScaling(*pTempColShape, a_Scaling->GetX(), a_Scaling->GetY(),
                                                 a_Scaling->GetZ());
