...
mCGotoRequestHandler::mCGotoRequestHandler() : m_uPutToGroundTries(0)
{}
...
void mCGotoRequestHandler::Process()
{
    if (m_uPutToGroundTries > 0)
    {
        ...
        if (PhysicsScene.TraceRayNearestHit(m_PutToGroundLocation, bCVector(0, -1, 0), pPlayer,
                                            eETraceRayHints_TestStaticShapes | eETraceRayHints_TestDynamicShapes
                                                | eETraceRayHints_Impact | eETraceRayHints_MaxDistanceNotFromDirection))
        {
