...
void mCEntityRequestHandler::HandleRequest()
{
    ...
    if (pEntity)
    {
        if (Request.has_moveto())
        {
            ...
            WorldMatrix.SetToRotation(
                bCEulerAngles(Pos.rotation().yaw(), Pos.rotation().pitch(), Pos.rotation().roll()));
            WorldMatrix.ModifyScaling(bCVector(Pos.scale().x(), Pos.scale().y(), Pos.scale().z()));
            WorldMatrix.ModifyTranslation(
                bCVector(Pos.translation().x(), Pos.translation().y(), Pos.translation().z()));
...
void mCEntityRequestHandler::Process()
{
    ...
    for (bTObjMap<bCPropertyID, mCPutToGroundController>::bCIterator Iter = m_putToGround.Begin();
         Iter != m_putToGround.End(); Iter++)
    {
        if (Iter->HasFinished())
...
mCEntityRequestHandler::mCPutToGroundController::mCPutToGroundController(bCPropertyID a_EntityID)
    : m_EntityID(a_EntityID), m_bCollisionEnabled(GEFalse), m_bAddedRigidBody(GEFalse), m_bAddedCollisionShape(GEFalse),
      m_OriginalColGroup(eECollisionGroup_Undefined), m_msEnableTimestamp(0)
{}
...
void mCEntityRequestHandler::mCPutToGroundController::EnableDynamicPhysics()
{
    ...
    // !pEntity->HasPropertySet(eEPropertySetType_MeshVisual)) return;

    if (!pEntity->HasPropertySet(eEPropertySetType_RigidBody))
    {
        bCPropertyObjectBase *pRigidBody =
            bCPropertyObjectSingleton::GetInstance().QueryNewObject(bTClassName<eCRigidBody_PS>::GetUnmangled());
    ...
    if (!pEntity->HasPropertySet(eEPropertySetType_CollisionShape))
    {
        bCPropertyObjectBase *pColShapePS =
            bCPropertyObjectSingleton::GetInstance().QueryNewObject(bTClassName<eCCollisionShape_PS>::GetUnmangled());
    ...
    if (m_bAddedCollisionShape)
    {
        ...
        pColShapePS->AddShape(eECollisionShapeType_Box, eEShapeGroup_Dynamic, BoxShape.GetCenter(),
                              BoxShape.GetExtent());
        ...
        eCResourceDataEntity * pMeshData = pResourceAdmin->QueryResourceDataEntity(strMeshName,
        eEResourceCacheBehavior_Immediate); eCResourceDataEntity * pColShapeData =
        pResourceAdmin->QueryNewResourceDataEntity();

        if(pMeshData && pColShapeData)
        {
            pColShapeData->CreateResourceFromType(bTClassName<eCResourceCollisionMesh_PS>::GetUnmangled());
            eCResourceCollisionMesh_PS * pColShape = dynamic_cast<eCResourceCollisionMesh_PS
        *>(pColShapeData->SearchResourcePropertySet()); if(pColShape)
            {
    ...
    /*if(PhysicsScene.TraceRayNearestHit(pEntity->GetWorldPosition(), bCVector(0, -2000, 0), pEntity,
    eETraceRayHints_TestStaticShapes | eETraceRayHints_TestDynamicShapes | eETraceRayHints_Impact))
    {
...
void mCEntityRequestHandler::mCPutToGroundController::RestorePhysics()
{
    ...
    if (!m_bAddedCollisionShape)
    {
        eCCollisionShape_PS *pColShapePS =
            GetPropertySet<eCCollisionShape_PS>(pEntity, eEPropertySetType_CollisionShape);
